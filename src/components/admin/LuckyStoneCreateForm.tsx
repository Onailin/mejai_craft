...
import { createLuckyStoneViaApi } from "@/lib/upload-lucky-stone-client";
...
export function LuckyStoneCreateForm() {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createLuckyStoneViaApi(formData);
      if (!result.ok) {
    ...
    >
      <div className="border-b border-stone-100 bg-gradient-to-r from-stone-50 to-violet-50/40 px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-semibold text-stone-900">เพิ่มหินมงคล</h2>
            <p className="text-xs text-stone-500">ชื่อ ความหมาย คำอธิบาย และรูป — แสดงบนหน้าแรก</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4">
          <label className="space-y-2">
            ...
            <input
              name="name"
              required
              placeholder="เช่น Onyx, Rose Quartz"
          ...
          <label className="space-y-2">
            ...
            <textarea
              name="description"
              rows={4}
              placeholder="รายละเอียดสั้นๆ เกี่ยวกับหิน"
          ...
          <label className="flex cursor-pointer items-center gap-2 text-sm text-stone-700">
            <input type="checkbox" name="isActive" defaultChecked className="h-4 w-4 rounded border-stone-300" />
            แสดงบนหน้าแรก
          </label>
